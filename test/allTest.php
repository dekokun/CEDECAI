...
class AllTest extends PHPUnit_Framework_TestCase
{
    ...
    public function allRemainPointsがそれっぽい動きをしている() {
        $rule = new \Rule\MonteCarlo();
        $this->assertEquals([40, 45, 45, 45], array_map(function($val) {return array_sum($val);}, $rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(1))));
        $this->assertEquals([36, 40, 40, 40], array_map(function($val) {return array_sum($val);}, $rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(2))));
        $this->assertEquals([0, 4, 4, 4], array_map(function($val) {return array_sum($val);}, $rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(10))));
        foreach($rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(10)) as $points) {
            foreach($points as $point) {
                $this->assertGreaterThanOrEqual(0, $point);
            }
        }
        $this->assertNotEquals($rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(10)), $rule->allRemainPoints(new Heroines([0, 0, 0, 0, 0, 0, 0, 0]), new \Turn(10)), 'こちら、ごくごく稀に落ちるので注意');
    }
}
